#!/usr/bin/env python3
"""
xi_jensen_contour_polyroots.py

Contour-integral + polyroots version of the Xi–Jensen scanner.

Main changes
------------
1. Taylor coefficients are extracted from Xi by a contour integral / circular
   sampling formula rather than repeated high-order differentiation.

       a_n = (1 / 2πi) ∮ Xi(z) z^{-(n+1)} dz

   Numerically, on a circle |z| = R:
       a_n ≈ (1/N) Σ_k Xi(R e^{2πik/N}) e^{-2π i n k / N} / R^n

2. Roots are computed with mpmath.polyroots instead of numpy.roots.

This is slower than numpy roots, but more faithful to the "use polyroots"
request and better aligned with high-precision work.
"""
...
def xi_gammas_contour(max_index: int, dps: int = 80, radius: float = 0.75, N: int | None = None) -> list[mp.mpf]:
    """
    Recover gamma(0),...,gamma(max_index) from Xi sampled on a circle.

    If Xi(z) = sum a_m z^m, then approximately
...
def roots_polyroots(coeffs_ascending: list[mp.mpf], maxsteps: int = 100, cleanup: bool = True) -> list[mp.mpc]:
    desc = [mp.mpc(c) for c in reversed(coeffs_ascending)]
    return mp.polyroots(desc, maxsteps=maxsteps, cleanup=cleanup)


# --------------------------------
# Relative-tolerance classification
# --------------------------------
...
PRESETS = {
    "c070_polyroots": {
        ...
        "real_rtol": 1e-8,
        "real_atol": 1e-12,
        "output_name": "xi_jensen_polyroots_c070.csv",
    },
    "c060_polyroots": {
        "mode": "threshold_ray",
        "c": 0.60,
        ...
        "real_rtol": 1e-8,
        "real_atol": 1e-12,
        "output_name": "xi_jensen_polyroots_c060.csv",
    },
    "fixed_d_20_polyroots": {
        "mode": "fixed_d",
        "c": None,
        "fixed_d": 20,
        "n_start": 20,
        ...
        "real_rtol": 1e-8,
        "real_atol": 1e-12,
        "output_name": "xi_jensen_polyroots_fixed_d20.csv",
    },
}


def parse_args():
    p = argparse.ArgumentParser(description="Contour + polyroots Xi–Jensen scanner")
...
def config_from_args(args):
    if args.preset:
        ...
        cfg = {
            ...
            "output_name": args.output_name if args.output_name is not None else "xi_jensen_polyroots_custom.csv",
        }
    else:
...
def main():
    ...
    for n in range(cfg["n_start"], cfg["n_stop"] + 1):
        ...
        written += 1
        if written % 5 == 0:
            print(f"Wrote {written} new rows so far... last n={n}")
